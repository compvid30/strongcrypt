...
const
  ...
  DEFAULT_KEY_ITERATIONS = 20000; // This was 2048 because of PDAs, but it way way too low
  DEFAULT_KEY_ITERATIONS_INCREMENT = 100;
...
{$ENDIF}
  ...
  if (status <> Active) then
    ...
    if status then
      ...
      if not(allOK) then
        begin
        raise EFreeOTFEConnectFailure.Create('StrongCrypt driver not installed/not running');
        end;
      ...
      if not(allOK) then
        begin
        raise EFreeOTFEConnectFailure.Create('Could not disconnect from StrongCrypt driver?!');
        end;
  ...
  if (Active) then
    ...
    if (Version() < FREEOTFE_ID_v00_58_0000) then
      ...
      raise EFreeOTFEObsoleteDriver.Create(
                     'An old version of the StrongCrypt driver was detected. Please upgrade before using this software'
                     );
      end;
...
begin
  ...
  if (volumeFilenames.count = 0) then
    ...
    if (IsLUKSVolume(volumeFilenames[0])) then
      begin
      Result := MountLUKS(
                          volumeFilenames,
                          mountedAs,
...
{$ENDIF}
  ...
  if (allOK) then
    ...
    if (volumeDetails.CDBFormatID > CDB_FORMAT_ID) then
      begin
      SDUMessageDlg(
                 _('WARNING: This volume appears to have been created with a later version of StrongCrypt than this software supports.'+SDUCRLF+
                 SDUCRLF+
    ...
    try
      ...
      try
        ...
        if (hashCount = 0) then
          begin
          SDUMessageDlg(
                        _('You do not appear to have any StrongCrypt hash drivers installed and started.')+SDUCRLF+
                        SDUCRLF+
                        _('If you have only just installed StrongCrypt, you may need to restart your computer.'),
                        mtError
                       );
          end;
        ...
        if (cypherCount = 0) then
          begin
          SDUMessageDlg(
                        _('You do not appear to have any StrongCrypt cypher drivers installed and started.')+SDUCRLF+
                        SDUCRLF+
                        _('If you have only just installed StrongCrypt, you may need to restart your computer.'),
                        mtError
                       );
