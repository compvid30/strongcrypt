...
uses
  ...
  SDUFrames,
  SDUSpin64Units,
  OTFEFreeOTFE_frmWizard,
  OTFEFreeOTFE_InstructionRichEdit,
  SDUDialogs,
  Gauges;
...
begin
  ...
  lblBits.Caption := IntToStr(bits) + ' Bits';
  Gauge1.Progress := round((100 * bits) / 128);

  case Bits of
    0..47: Gauge1.ForeColor := $000000FF;
    48..95: Gauge1.ForeColor := $0000AAFF;
    96..2048: Gauge1.ForeColor := $0000AA00;
  end;
...
begin
  Result := (rgFileOrPartition.ItemIndex =
    rgFileOrPartition.Items.IndexOf(FILEORPART_OPT_PARTITION));
end;
...
begin
  ...
  begin
    retVal :=
      TFreeOTFESectorIVGenMethod(cbSectorIVGenMethod.Items.Objects[cbSectorIVGenMethod.ItemIndex]);
  end;
...
begin
  ...
  Result := Copy(fCombinedRandomData, (CRITICAL_DATA_LENGTH div 8) + 1, (TempCypherUseKeyLength div
    8));
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if not (FreeOTFEObj.GetSpecificHashDetails(hashKernelModeDriverNames[i],
          StringToGUID(hashGUIDs[i]), hashDetails)) then
        begin
...
begin
  ...
  if VerInfo.dwMajorVersion >= 6 then
    lblWelcomebanner.Font.Name := 'Segoe UI'
  else
    lblWelcomebanner.Font.Name := 'Tahoma';
end;
...
begin
  ...
  if ckRNGIsaac.checked then
  begin
    if (RNGs = '') then
    begin
      RNGS := RNGs + ckRNGIsaac.caption;
    end
    else
    begin
      RNGS := RNGs + ', ' + ckRNGIsaac.caption;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          fileCreateProbMsg :=
            SDUParamSubstitute(_('Unable to create volume file; please ensure you have %1 free on the relevant drive'), [SDUFormatAsBytesUnits(volumeFileSize)]);
...
begin
  ...
  else if not (FreeOTFEObj.GetSpecificCypherDetails(
    CypherDriver,
    CypherGUID,
    cypherDetails
    )) then
...
begin
  ...
  lblPartitionDiskSize.Caption := SDUParamSubstitute(_('(Approx: %1)'),
...
begin
  ...
  reInstructWelcome.Text :=
    _('This "wizard" will guide you through the process of either creating a new StrongCrypt volume file, or a hidden volume within an existing file.'
      + SDUCRLF +
    SDUCRLF +
  ...
  reInstructFileOrPartition.Text :=
    _('Please specify whether the new volume should be created as a disk image file stored on your filesystem, or take up a disk partition/entire physical disk'
      + SDUCRLF +
    SDUCRLF +
  ...
  reInstructFilename.Text :=
    _('Please specify the filename and location where the new volume should be created, by clicking the browse button.'
      + SDUCRLF +
    SDUCRLF +
  ...
  reInstructOffset.Text :=
    _('The volume file you specified already exists. To create a hidden volume within this file, please specify the byte offset within this file from where the new volume should begin.'
      + SDUCRLF +
    SDUCRLF +
