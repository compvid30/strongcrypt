...
procedure TfrmMain.actLicenseExecute(Sender: TObject);
var
  dlg: TfrmLicense;
begin
  dlg := TfrmLicense.Create(self);
  try
    dlg.ShowModal();
  finally
    dlg.Free();
...
begin
  if ShellExecute(handle, 'open', PChar('http://www.strongcrypt.org'), nil, nil, SW_SHOWNORMAL) <= 32 then
  SDUMessageDlg(_('Unable to open the website'), mtError);
end;
