...
begin
  ...
  if allOK then
    ...
    if not(allOK) then
      ...
      SDUMessageDlg(
                    ...
                    SDUCRLF+
                    _('Please either select another location for storing your settings (e.g. your user profile), or move the StrongCrypt executable such that it is not stored underneath the directory shown above.'),
                    mtWarning
                   );
  ...
  if allOK then
    ...
    if (
      ...
      begin
      allOK := (SDUMessageDlg(
                              _('You have not specified a location where StrongCrypt should save its settings to.')+SDUCRLF+
                              SDUCRLF+
                              _('Although the settings entered will take effect, they will revert back to their defaults when StrongCrypt is exited.')+SDUCRLF+
                              SDUCRLF+
    ...
    // old settings file
      ...
      if (
        ...
        deleteOldLocation := (SDUMessageDlg(
                                            msgSegment+SDUCRLF+
                                            SDUCRLF+
                                            _('Would you like StrongCrypt to delete your previous settings file?')+SDUCRLF+
                                            SDUCRLF+
                                            _('Note: If you select "No" here, StrongCrypt may pick up your old settings the next time you start FreeOTFE'),
                                            mtConfirmation,
                                            [mbYes, mbNo],
                                            0
                                           ) = mrYes);
