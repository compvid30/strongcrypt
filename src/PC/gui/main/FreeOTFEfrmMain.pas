...
type
  ...
  TfrmFreeOTFEMain = class(TfrmMain)
    ...
    ilDisabled_Large: TImageList;
    ilDisabled_Small: TImageList;
    Timer1: TTimer;
    pnlBase: TPanel;
    lblNotice: TLabel;
    lvDrives: TListView;
  ...
  protected
    ...
    procedure MountFiles(mountAsSystem: TDragDropFileType; filenames: TStringList; readOnly:
    ...
    procedure UACEscalateForPortableMode(portableAction: TPortableModeAction; suppressMsgs:
      boolean);
...
var
  // We need this to close the Explorer windows before dismount
  ExplorerWND: string;
...
resourcestring
  FREEOTFE_DESCRIPTION = 'StrongCrypt: Disk Encryption System';

  TEXT_NEED_ADMIN = 'You need administrator privileges in order to carry out this operation.';

  // Toolbar captions...
  RS_TOOLBAR_CAPTION_MOUNTPARTITION = 'Mount disk';
...
procedure TfrmFreeOTFEMain.DrawWatermark();
var
  lv: TLVBKIMAGE;
  hinst: Thandle;
const
...
begin
  ...
  lv.hbm := Image1.Picture.Bitmap.Handle;

  SendMessage(lvDrives.Handle, LVM_SETBKIMAGE, 0, integer(@lv));
end;
  ...
  begin
    ...
    begin
      goForStartPortable := SDUConfirmYN(
        _('The main StrongCrypt driver does not appear to be installed/running on this computer') +
          SDUCRLF +
        SDUCRLF +
        _('Would you like to start StrongCrypt in portable mode?')
        );
    end;
    ...
    begin
      SDUMessageDlg(
        _('Please see the "installation" section of the accompanying documentation for instructions on how to install the StrongCrypt drivers.'),
        mtInformation,
        [mbOK],
        0
        );
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          cypherstring := OTFEFreeOTFE.GetCypherDisplayTitle(cypherDetails);
        end;

        ListItem.SubItems.Add(cypherstring);
...
begin
  ...
  begin
    if ((pmSystemTray.Items[i].Tag and TAG_SYSTRAYICON_POPUPMENUITEMS) =
      TAG_SYSTRAYICON_POPUPMENUITEMS) then
    begin
...
begin
  ...
  begin
    ...
    ToolBar1.DisabledImages := ilDisabled_Large;
  ...
  begin
    ...
    ToolBar1.DisabledImages := ilDisabled_Small;
...
begin
  ...
  IconMounted.Handle := LoadImage(hInstance, PChar(R_ICON_MOUNTED), IMAGE_ICON, 16, 16,
    LR_DEFAULTCOLOR);
  IconUnmounted := TIcon.Create();
  IconUnmounted.Handle := LoadImage(hInstance, PChar(R_ICON_UNMOUNTED), IMAGE_ICON, 16, 16,
    LR_DEFAULTCOLOR);
...
procedure TfrmFreeOTFEMain.FormShow(Sender: TObject);
begin
  inherited;
...
begin
  ...
  //    tmpColumn.width := (lvDrives.clientwidth - lvDrives.columns[0].width - ilDriveIcons.width) + 10;
  //    end;
...
procedure TfrmFreeOTFEMain.MountFiles(mountAsSystem: TDragDropFileType; filenames: TStringList;
  readOnly: boolean);
var
  i: integer;
  mountedAs: string;
...
begin
  ...
  begin
    ...
    begin
      msg := SDUParamSubstitute(_('Your StrongCrypt volume has been mounted as drive: %1'),
        [prettyMountedAs]);
    end
    else
    begin
      msg := SDUParamSubstitute(_('Your StrongCrypt volumes have been mounted as drives: %1'),
        [prettyMountedAs]);
    end;
...
// Quick and Dirty. Can be done better.
// Just kill all windows, because Windows 8 dismount does not like open windows.
function CloseExplorerWindows(wHandle: HWND; Param : Cardinal): Bool; stdcall; export;
var
...
begin
 ...
 if ClassName = 'CabinetWClass' then
    begin
      wndChild := FindWindowEx(wHandle, 0, 'WorkerW', nil);
      wndChild := FindWindowEx(wndChild, 0, 'ReBarWindow32', nil);
      wndChild := FindWindowEx(wndChild, 0, 'Address Band Root', nil);
      wndChild := FindWindowEx(wndChild, 0, 'msctls_progress32', nil);
      wndChild := FindWindowEx(wndChild, 0, 'Breadcrumb Parent', nil);
      wndChild := FindWindowEx(wndChild, 0, 'ToolbarWindow32', nil);

      if wndChild <> 0 then
      begin
//        p := StrAlloc(300);
//        SendMessage(wndChild, WM_GETTEXT, WParam(300), LParam(p));
//
//        if pos(lowercase(ExplorerWND), lowercase(p)) > 0 then
...
begin
  ...
  EnumWindows(@CloseExplorerWindows, 0);
  sleep(300);
...
begin
  ...
  if (StatusBar_Status.Visible) then
  begin
    statusText := '';
    if not (OTFEFreeOTFE.Active) then
    begin
      statusText := _('StrongCrypt main driver not connected.');
    end
...
procedure TfrmFreeOTFEMain.AutoRunExecute(autorun: TAutorunType; driveLetter: char; isEmergency:
  boolean);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SDUMessageDlg(
          SDUParamSubstitute(_('Unable to locate %1 executable:'), [AUTORUN_TITLE[autorun]]) +
            SDUCRLF +
          SDUCRLF +
          exeOnly,
          mtWarning
...
begin
  ...
  begin
    SDUMessageDlg(
      SDUParamSubstitute(_('The main StrongCrypt driver installed on this computer dates back to StrongCrypt %1'), [OTFEFreeOTFE.VersionStr()]) + SDUCRLF +
      SDUCRLF +
      _('It is highly recommended that you upgrade your StrongCrypt drivers to v3.00 or later as soon as possible, in order to allow the use of LRW and XTS based volumes.') + SDUCRLF +
      SDUCRLF +
  ...
  begin
    SDUMessageDlg(
      SDUParamSubstitute(_('The main StrongCrypt driver installed on this computer dates back to StrongCrypt %1'), [OTFEFreeOTFE.VersionStr()]) + SDUCRLF +
      SDUCRLF +
      _('It is highly recommended that you upgrade your StrongCrypt drivers to those included in v4.30 or later as soon as possible, due to improvements in the main driver.') + SDUCRLF +
      SDUCRLF +
      _('See documentation (installation section) for instructions on how to do this.'),
      mtWarning
      );
...
function TfrmFreeOTFEMain.PortableModeSet(setTo: TPortableModeAction; suppressMsgs: boolean):
  boolean;
var
  retval: boolean;
begin
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SDUMessageDlg(
          _('Unable to locate any portable StrongCrypt drivers.') + SDUCRLF +
          SDUCRLF +
          _('Please ensure that the a copy of the StrongCrypt drivers (".sys" files) you wish to use are located in the correct directory.'),
          mtWarning
          );
      end;
    end
    ...
    begin
      ...
      begin
        ...
        begin
          SDUMessageDlg(
            _('One or more of your portable StrongCrypt drivers could not be installed/started.') +
              SDUCRLF +
            SDUCRLF +
    ...
    begin
      ...
      begin
        stopOK := (SDUMessageDlg(
          ...
          SDUCRLF +
          _('If any of the currently mounted volumes makes use of any of the StrongCrypt drivers which are currently in portable mode, stopping portable mode is not advisable.') + SDUCRLF +
          SDUCRLF +
          _('It is recommended that you dismount all volumes before stopping portable mode.') +
            SDUCRLF +
          SDUCRLF +
  ...
  begin
    ...
    begin
      ...
      begin
        if not (suppressMsgs) then
        begin
          SDUMessageDlg(_('Portable mode drivers stopped and uninstalled.'), mtInformation, [mbOK],
            0);
        end;
      end;
    ...
    begin
      ...
      begin
        SDUMessageDlg(
          _('One or more of your portable StrongCrypt drivers could not be stopped/uninstalled.') +
            SDUCRLF +
          SDUCRLF +
          TEXT_NEED_ADMIN + SDUCRLF +
          SDUCRLF +
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        userConfirm := SDUMessageDlg(
          _('One or more of the StrongCrypt drivers are running in portable mode.') + SDUCRLF +
          SDUCRLF +
...
{$ENDIF}
      ...
      begin
        ...
        begin
          msg := msg + SDUCRLF +
            SDUCRLF +
            _('Another instance of StrongCrypt is running - the hotkey may already be assigned to that instance.');
          msgType := mtWarning;
        end;
...
procedure TfrmFreeOTFEMain.Timer1Timer(Sender: TObject);
begin
  inherited;
...
procedure TfrmFreeOTFEMain.ToolBar1CustomDraw(Sender: TToolBar;
...
function TfrmFreeOTFEMain.HandleCommandLineOpts_DriverControl_Install(driverControlObj:
  TOTFEFreeOTFEDriverControl): integer;
var
  cmdExitCode: integer;
  paramValue: string;
...
function TfrmFreeOTFEMain.HandleCommandLineOpts_DriverControl_Uninstall(driverControlObj:
  TOTFEFreeOTFEDriverControl): integer;
var
  cmdExitCode: integer;
  paramValue: string;
...
function TfrmFreeOTFEMain.HandleCommandLineOpts_DriverControl_Count(driverControlObj:
  TOTFEFreeOTFEDriverControl): integer;
var
  cmdExitCode: integer;
  paramValue: string;
...
begin
  ...
  begin
    if (OTFEFreeOTFE.LastErrorCode <> OTFE_ERR_USER_CANCEL) then
    begin
      SDUMessageDlg(_('StrongCrypt volume could not be created'), mtError);
    end;
  end
  else
  begin
    ...
    begin
      msg := _('StrongCrypt volume created successfully.') + SDUCRLF +
        SDUCRLF +
    ...
    begin
      ...
      msg := SDUParamSubstitute(
        _('StrongCrypt volume created successfully and mounted as: %1.'),
        [createdMountedAs]
        ) + SDUCRLF +
        SDUCRLF +
